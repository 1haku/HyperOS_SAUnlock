...
バックアップはこの下の `devices/<device-id>/slot-<0-or-1>/` に分けて保存されます。ID はシリアル番号の UTF-8 バイト列を16進数にしたもので、機種名もファイル内に保存します。従来の単一ファイルは対応するフォルダーへ移動し、既存ファイルは上書きしません。
...
※端末・スロットを切り替えると、その対象のバックアップが自動的に選ばれます。Settings 値は端末全体で共有されるため、両スロットを操作した場合は逆順で復元し、SIM の構成とデータ通信用 SIM は変更しないでください。未完了の復旧は元のスロットで済ませてから、同じ端末の設定を変更してください。`-BackupDirectory` で保存先を変更した場合は、復元時も同じ保存先を指定してください。

※`backup.json` には端末のシリアル番号が含まれるため、共有・公開しないよう注意してください。

## トラブルシューティング

設定の書き込み権限がない場合は、端末の **USB デバッグ（セキュリティ設定）** を有効にしてください。通常の USB デバッグとは別のスイッチです。復旧時は値が一致する項目への書き込みを省き、全項目の読み戻し結果で復旧の成否を判定します。
...
*※端末側の仕様詳細、動作確認済み機種、免責事項は [メイン README](../README.ja.md) を参照してください。*
